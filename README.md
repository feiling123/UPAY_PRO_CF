...
方式 B：只把 cloudflare/ 目录里的内容上传到仓库根目录
...
5. 保持默认 Build command：`npm run build`。
6. 保持默认 Deploy command：`npm run deploy`。
7. 确认部署。Cloudflare Workers Builds 会安装依赖、构建、执行 D1 migration，并部署 Worker。
8. 部署成功后打开 Worker 域名，访问 `/api/health`，应返回 `ok: true`。
9. 进入 `ADMIN_PATH` 对应的隐藏后台入口，用初始管理员 secrets 登录并完成初始化。
...
7. 在 Worker Variables 中把 `ADMIN_PATH` 改成自己的隐藏后台目录，例如 `/a9f3-upay-console`，不要使用 `/admin`、`/login`、`/api`、`/pay` 等保留路径。
...
`ADMIN_PATH` 不写死在代码里，必须在 Cloudflare Dashboard 的 Worker Variables 中设置：

```text
ADMIN_PATH=/a9f3-upay-console
```

`ADMIN_PATH` 是后台唯一公开入口。部署到生产前必须改成自己的随机路径，访问方式为：
...
如果未设置 `ADMIN_PATH`，后台入口不可访问，固定 `/admin`、`/admin.html`、`/login.html` 也都会返回 nginx 风格 404。
